...
// FIXED: aligns DocType/Id with Cosmos repository and uses existing Models/* types.
using System.Net;
...
public sealed class EstimatesSyncFunctions
{
    ...
    public EstimatesSyncFunctions(CosmosClient cosmos) => _cosmos = cosmos;
        ...
        FunctionContext context)
    {
        ...
        try
        {
            var result = await RunSyncAsync(body.SinceUtc, body.IncludeInactive ?? false, body.ProjectId, log);
        ...
        {
            log.LogError(ex, "SyncEstimatesNow failed. sinceUtc={SinceUtc} includeInactive={IncludeInactive} projectId={ProjectId}",
                body.SinceUtc, body.IncludeInactive, body.ProjectId);

            var resp = req.CreateResponse(HttpStatusCode.BadRequest);
            await resp.WriteAsJsonAsync(new { ok = false, serverUtc = DateTime.UtcNow.ToString("o"), message = ex.Message });
            return resp;
        }
    }

    [Function("Estimates_Sync_Timer")]
    public async Task EstimatesSyncTimer([TimerTrigger("0 */30 * * * *")] TimerInfo timer, FunctionContext context)
    {
        ...
        try
        {
            var result = await RunSyncAsync(sinceUtc, includeInactive: false, projectId: null, log);
            result.ElapsedMs = (long)(DateTime.UtcNow - startedUtc).TotalMilliseconds;

            log.LogInformation("Estimates sync timer complete. ok={Ok} headersUpserted={Headers} detailsUpserted={Details} elapsedMs={ElapsedMs}",
                result.Ok, result.HeadersUpserted, result.DetailsUpserted, result.ElapsedMs);
        }
        catch (Exception ex)
        {
            log.LogError(ex, "Estimates sync timer failed. sinceUtc={SinceUtc}", sinceUtc);
        }
    }

    private async Task<SyncResult> RunSyncAsync(string? sinceUtc, bool includeInactive, int? projectId, ILogger log)
    {
        var headersUrl = Environment.GetEnvironmentVariable("ESTIMATES_SYNC_URL");
        var detailsUrl = Environment.GetEnvironmentVariable("ESTIMATEDETAILS_SYNC_URL");

        if (string.IsNullOrWhiteSpace(headersUrl)) throw new InvalidOperationException("Missing app setting ESTIMATES_SYNC_URL.");
        if (string.IsNullOrWhiteSpace(detailsUrl)) throw new InvalidOperationException("Missing app setting ESTIMATEDETAILS_SYNC_URL.");
        ...
        var payload = new { sinceUtc, includeInactive, projectId };

        using var http = new HttpClient { Timeout = TimeSpan.FromMinutes(5) };

        // --- Headers
        var headersStarted = DateTime.UtcNow;
        using var headersResp = await http.PostAsJsonAsync(headersUrl, payload);
        ...
        foreach (var row in headersArr.EnumerateArray())
        {
            ...
            if (string.IsNullOrWhiteSpace(estimateId)) continue;

            var doc = new PmwebEstimateHeaderDoc
            {
                Id = $"pmwebEstimate|{estimateId}",
                TenantId = tenantId,
                DocType = "pmwebEstimateHeader",
        ...
        // --- Details
        using var detailsResp = await http.PostAsJsonAsync(detailsUrl, payload);
        ...
        foreach (var row in detailsArr.EnumerateArray())
        {
            ...
            if (string.IsNullOrWhiteSpace(detailId) || string.IsNullOrWhiteSpace(estimateId)) continue;

            var doc = new PmwebEstimateDetailDoc
            {
                Id = $"pmwebEstimateDetail|{detailId}",
                TenantId = tenantId,
                DocType = "pmwebEstimateDetail",
                DetailId = detailId,
                EstimateId = estimateId,
        ...
        log.LogInformation("Estimates sync complete. Upserted headers={Headers} details={Details} into {Db}/{Container} tenant={TenantId}",
        ...
        return new SyncResult
        {
            ...
            HeadersElapsedMs = (long)(DateTime.UtcNow - headersStarted).TotalMilliseconds,
            DetailsElapsedMs = 0
        };
    }
